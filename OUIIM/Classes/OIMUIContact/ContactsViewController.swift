...
//        viewModel.newGroupCountRelay.map { "\($0)" }.bind(to: groupNotiCell.badgeLabel.rx.text).disposed(by: _disposeBag)
        viewModel.newGroupCountRelay.map({ "\($0)" }).subscribe(onNext:{ [weak self] in
            self?.groupNotiCell.badgeLabel.text = $0
        }).disposed(by: _disposeBag)
        viewModel.newFriendCountRelay.map({ "\($0)" }).subscribe(onNext:{ [weak self] in
            self?.newFriendCell.badgeLabel.text = $0
        }).disposed(by: _disposeBag)
