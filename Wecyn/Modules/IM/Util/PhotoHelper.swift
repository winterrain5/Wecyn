...
//        if redirect {
        ...
        _ = originPath.md5 + ".\(originPath.split(separator: ".").last!)"
...
//            } else {
                Toast.showLoading()
                let task = connection.dataTask(with: URLRequest(url: URL(string: originPath)!)) { [weak self] data, responose, error in
                    guard let `self` = self else { return }
                    if let path = responose?.url?.relativeString, let url = URL(string: path) {
                        DispatchQueue.main.async {
                            Toast.dismiss()
                            print("重定向地址：\(url)")
                            self.browserHelper(paths: [path], thumbsURL: thumbsURL, forVideo: forVideo, index: index)
                        }
