//
//  CustomContactMessageCell.swift
//  Wecyn
//
...
class CustomContactMessageCell: MessageCollectionViewCell {
    // MARK: Lifecycle
    ...
    func configure(
        ...
        and sizeCalculator: CustomContactMessageLayoutSizeCalculator)
    {
        ...
        if case MessageKind.custom(let custom) = message.kind,let contact = custom as? IMContactItem {
            
            let name = contact.displayName
            let wid = "wid:\(contact.wid)"
            if fromCurrentSender {
                ...
                let bottomLabelAttr = NSMutableAttributedString(string: wid)
                bottomLabelAttr.addAttribute(.foregroundColor, value: UIColor.white.withAlphaComponent(0.8), range: NSRange(location: 0, length: wid.count))
                ...
                let bottomLabelAttr = NSMutableAttributedString(string: wid)
                bottomLabelAttr.addAttribute(.foregroundColor, value: UIColor.lightGray, range: NSRange(location: 0, length: wid.count))
                cellTopLabel.attributedText = topLabelAttr
                cellBottomLabel.attributedText = bottomLabelAttr
            }
            cellImageView.kf.setImage(with: contact.faceUrl?.url,placeholder: R.image.proile_user())
...
class CustomContactMessageLayoutSizeCalculator: CellSizeCalculator {
    // MARK: Lifecycle
    ...
    -> CGSize
    {
        if case MessageKind.custom(let custom) = message.kind {
            if let contact = custom as? IMContactItem {
    ...
    -> CGSize
    {
        ...
        if case MessageKind.custom(let custom) = message.kind {
            if let contact = custom as? IMContactItem {
                let wid = "wid:\(contact.wid)"
                let widAttrSize = NSAttributedString(string: wid).size(consideringWidth: 120)
                let height = widAttrSize.height
