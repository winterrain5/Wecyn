...
class EditorDemoController: BaseViewController {    
    ...
    var editComplete:((_ text:String,_ html:String)->())?
    ...
    func complete() {
        if self.editorView.richTextView.text.isEmpty {
            self.returnBack()
            return
        }
        let text = self.editorView.richTextView.text ?? ""
        let html = self.editorView.richTextView.getHTML()
        self.editComplete?(text,html)
        self.returnBack()
    }
