...
class AdminRoomController: BasePagingTableController {
    ...
    override func refreshData() {
        ...
        AdminService.adminRoomList(orgId: Admin_Org_ID,keyword: "").subscribe(onNext:{
            self.dataArray = $0
            self.endRefresh($0.count,emptyString: "No Room")
    ...
    override func createListView() {
        ...
        tableView?.register(nibWithCellClass: AdminRoomCell.self)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withClass: AdminRoomCell.self)
        
        if self.dataArray.count > 0 {
            cell.model = self.dataArray[indexPath.row] as? AdminRoomModel
        }
    ...
    func editRole(_ item:AdminRoomModel) {
        let vc = AdminAddRoomController(model: item)
        let nav = BaseNavigationController(rootViewController: vc)
        nav.modalPresentationStyle = .fullScreen
        self.present(nav, animated: true)
    }
    
    func deleteRole(_ item:AdminRoomModel) {

        func deleteRow(_ item:AdminRoomModel) {
            let idx = (self.dataArray as! [AdminRoomModel]).firstIndex(of: item) ?? 0
            self.dataArray.remove(at: idx)
            self.tableView?.deleteRows(at: [IndexPath(row: idx, section: 0)], with: .automatic)
        }
        let alert = UIAlertController(title: "Are you sure you want to delete this room?",message: nil, preferredStyle: .actionSheet)
        alert.addAction(title: "Delete directly",style: .default) { [weak self] _ in
            guard let `self` = self else { return }
            AdminService.deleteRoom(id: item.id).subscribe(onNext:{
