...
public extension ExtensionBase where Base: UIImage {
    ...
    ///   - imageSize: 图片的大小
    /// - Returns: 剪切后的图片
    func roundCorner(radius: CGFloat = 3, byRoundingCorners corners: UIRectCorner = .allCorners, imageSize: CGSize?) -> UIImage? {
    ...
    func circleImage() -> UIImage? {
        return roundCorner(radius: (self.base.size.width < self.base.size.height ? self.base.size.width : self.base.size.height) / 2.0, byRoundingCorners: .allCorners, imageSize: self.base.size)
    }
    ...
    func solidTo(size: CGSize) -> UIImage? {
        ...
        if self.base.size.height <= self.base.size.width {
            if self.base.size.width >= w {
                let scaleImage = fixOrientation().sk.scaleTo(size: CGSize(width: w, height: w * self.base.size.height / self.base.size.width))
                return scaleImage
            } else {
            ...
            if self.base.size.height >= h {
                let scaleImage = fixOrientation().sk.scaleTo(size: CGSize(width: h * self.base.size.width / self.base.size.height, height: h))
                return scaleImage
            } else {
                return fixOrientation()
            }
...
public extension ExtensionBase where Base: UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(base.size))?.sk.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
            let data = resizeIO(resizeSize: mode.resize(self.base.size))?.sk.compressDataSize(maxSize: mode.maxDataSize)
            DispatchQueue.main.async {
...
public extension ExtensionBase where Base: UIImage {
    ...
    private static func getHDImgWithCIImage(with image: CIImage, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
        ...
        if let newLogoRoundCorner = logoRoundCorner, let roundCornerLogo = logoImage.sk.roundCorner(radius: newLogoRoundCorner, byRoundingCorners: .allCorners, imageSize: logoSize) {
